...
MYSQL_CONTAINER=$(docker run -d -P -e EXTRA_OPS="--lower_case_table_names=1" -e MYSQL_USER=$DB_USERNAME -e MYSQL_PASS=$DB_PASSWORD -e ON_CREATE_DB=$DB_DATABASE -e MYSQL_ROOT_PASSWORD=$DB_PASSWORD tutum/mysql)
...
export DATABASE="jdbc:mysql://$DB_HOST:$DB_PORT/$DB_DATABASE?user=$DB_USERNAME&password=$DB_PASSWORD&allowMultiQueries=true&zeroDateTimeBehavior=convertToNull&createDatabaseIfNotExist=true"
